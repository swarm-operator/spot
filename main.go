...
const (
	...
	thresholdInterval    time.Duration = 2 * time.Second
)
...
func lookupEC2InstanceMetadata() (timestamp time.Time, err error) {
...
const (
	// manager hosts exposed as docker-api in control network
...
func main() {
	...
	for {
		...
		t, err := lookupEC2InstanceMetadata()
		if err == nil {
			if !t.IsZero() {
				err = drain(self)
				if err != nil {
				...
				fmt.Println("Set self to be drained ...")
